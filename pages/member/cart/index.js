...
Page({
  ...
  get_cart: function () {
    ...
    e.get("member/cart/get_cart", {}, function (e) {
      t = {
        ...
        total: e.total,
        cartcount: e.total,
        totalprice: Math.round(100*e.totalprice)/100,
        empty: e.empty || !1
  ...
  tapNum: function (ev) {
    ...
    var optionid = ev.currentTarget.dataset.optionid;
    ...
    e.post("member/cart/update", params, function (t) {
      that.get_cart()
    })
  },
  tapPlus: function (ev) {
    ...
    var optionid = ev.currentTarget.dataset.optionid;
    var max = parseInt(ev.currentTarget.dataset.max);
    var num = parseInt(ev.currentTarget.dataset.num);
    if(num < max){
      num += 1;
    }else{
      i.toast(that, '已超出最大购买量')
